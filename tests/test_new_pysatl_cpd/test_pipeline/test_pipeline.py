...
class TestPipeline:
    ...
    def test_check_two_steps(self):
        ...
        step1 = DataGenerationStep(
            ...
            output_storage_names={"data1", "data2"},
            output_step_names={"meta1", "meta2"}
        )
        ...
        step2 = ReportGenerationStep(
            self.mock_reporter,
            input_storage_names={"result1", "result2"},
            input_step_names={"meta3", "meta4"}
        )
        pipeline._check_two_steps(step1, step2)  # Should not raise any exception
        ...
        step2 = ExperimentExecutionStep(
            self.mock_worker,
            input_storage_names={"new_name"},
            input_step_names={"new_meta"}
        )
        pipeline._check_two_steps(step1, step2)  # Should not raise any exception
    ...
    def test_config_pipeline(self):
        ...
        step1 = DataGenerationStep(
            self.mock_data_handler,
            output_storage_names={"data1"},
            output_step_names={"meta1"}
        )
        ...
        step4 = ReportGenerationStep(
            self.mock_reporter,
            input_storage_names={"result2"},
            input_step_names={"meta3"}
        )
